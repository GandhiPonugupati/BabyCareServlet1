...
public class BabyDao {
	...
	public static int save(Baby b){
		...
		try{
			...
			PreparedStatement ps=con.prepareStatement("insert into javatpoint.babyname(name,meaning,sex,religion) values(?,?,?,?)");
	...
	public static int delete(int id){
		...
		try{
			Connection con=getCon();
			PreparedStatement ps=con.prepareStatement("delete from javatpoint.babyname where id=?");
			ps.setInt(1,id);
			status=ps.executeUpdate();
			con.close();
		}catch(Exception e){System.out.println(e);}
	...
	public static List<Baby> getAllRecords(){
		...
		try{
			Connection con=getCon();
			PreparedStatement ps=con.prepareStatement("select * from javatpoint.babyname");
	...
	public static List<Baby> getRecordsByStart(String start){
		...
		try{
			Connection con=getCon();
			PreparedStatement ps=con.prepareStatement("select * from javatpoint.babyname where name like '"+start+"%' ");
			ResultSet rs=ps.executeQuery();
			while(rs.next()){
				Baby b=new Baby();
				b.setId(rs.getInt(1));
	...
	public static List<Baby> getRecordsByReligion(String religion){
		...
		try{
			Connection con=getCon();
			PreparedStatement ps=con.prepareStatement("select * from javatpoint.babyname where religion=? ");
